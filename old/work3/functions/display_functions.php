<?php
function displayTotalSearchResults($searchData) {
    $totalSearchResults = count($searchData);
    return 'Total search results: ' . $totalSearchResults;
}

function displayTotalElements($storageController) {
    $totalElements = $storageController->getTotalElementsCount();
    return '<p class="text-right">Total elements: ' . $totalElements . '</p>';
}

function displayStoredElements($data, $startFrom, $totalPages, $sort, $searchColumn, $searchValue, $page) {
    if (!empty($data)) {
